...
// ---------------------------------------------------------------------------
// Violations API request body types — used by GetViolations.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Violation types — used by GetViolations (XrayService) and the test suite.
// ---------------------------------------------------------------------------
	...
	for artIdx, art := range parsed.Artifacts {
		...
		for _, issue := range art.Issues {
			...
			if existing, ok := seen[issue.IssueID]; ok {
				...
				if label != "" && !slices.Contains(existing.Platforms, label) {
...
// JWT tokens are properly initialized and retries/backoff are inherited
...
// generated by CreateInitialRefreshableTokensIfNeeded.
...
func (xs *XrayService) GetViolations(watchName, repo, path, projectKey string) ([]violationWithMalicious, error) {
	...
	for _, v := range allViolations {
		...
		if v.Properties != nil {
			if propsMap, ok := v.Properties.(map[string]any); ok {
